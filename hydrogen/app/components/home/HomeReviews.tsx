    ...
    <section className="bg-charcoal py-10 md:py-14">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-10 text-center">
          <p className="mb-1 text-xs font-bold uppercase tracking-[0.2em] text-crimson">Verified Buyers</p>
          ...
          {averageRating > 0 && (
            <div className="mt-4 flex items-center justify-center gap-3">
              ...
              {totalCount > 0 && (
                <span className="text-sm text-white/60">({totalCount.toLocaleString()} reviews)</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {reviews.slice(0, 6).map((review) => (
            <div
              key={review.id}
              className="flex flex-col gap-3 rounded-2xl border border-white/15 bg-white/[0.06] p-5"
            >
              ...
              {review.body && (
                <p className="line-clamp-3 text-sm leading-relaxed text-white/70">{review.body}</p>
              )}
            </div>
          ))}
        </div>
